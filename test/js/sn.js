;(function (init) {
    ...
    $a87C$var$_createClass(ShootingStar, [
      ...
      },
      {
        ...
        value: function start() {
          ...
          let start = new $a87C$export$default1()
          start.start()
        },
      },
    ])
  ...
  var $a87C$export$default1 = (function () {
    function ShootingStar() {
      ...
      // 初始化一些流星位置数据
      for (var i = 0; i < $a87C$var$COUNT1; i++) {
        positions.push(Math.random(), Math.random(), Math.random()) // 随机x、y、z坐标
        // positions.push(Math.floor(Math.random() * w) - w / 2, -h / 2, 1) // 随机x、y、z坐标
        mouse.push(-1, -1, 0, 0)
        aFront.push(front.x, front.y)
        random.push(Math.random())
      }
        ...
        value: function update(timestamp) {
          ...
          for (let i = 0; i < positions.length; i += 3) {
            ...
            if (positions[i + 1] > window.innerHeight / 2) {
              positions[i + 1] = -window.innerHeight / 2 // 重置y坐标到顶部
              positions[i] =
  ...
  if (window.innerWidth > 1000) {
    var $Focm$var$webGL1 = new $Focm$var$WebGL({
      canvas: document.getElementById('canvas'),
      container: document.querySelector('.canvas-el'),
    })
    setTimeout(function () {
      $Focm$var$webGL1.start()
    }, $Focm$var$DELAY)
  }
  return {
    Focm: {},

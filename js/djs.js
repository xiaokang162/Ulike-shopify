jQuery(function () {
  ...
  var g = function (b) {
    var c = b.children('.panel'),
      ...
      m =
        'auto' == c.find('.widthType[checked]').val()
          ? 0
          : c.find('.width').slider('value'),
    ...
    6 != d.split('/').join(' ').split(':').join(' ').split(' ').length
      ? c.find('.timeText').addClass('error')
      : c.find('.timeText').removeClass('error')
    isNaN(f)
      ? c.find('.timeZone').addClass('error')
      : c.find('.timeZone').removeClass('error')
    g
      ? c.find('.reflectionOpacity').parents('.group').show()
      : c.find('.reflectionOpacity').parents('.group').hide()
    g
      ? c.find('.reflectionBlur').parents('.group').show()
      : c.find('.reflectionBlur').parents('.group').hide()
    b.jCountdown({
      timeText: d,
      timeZone: f,
      style: a,
  ...
  jQuery('.page').each(function (b, c) {
    ...
    0 == d.width
      ? a.find('.widthType[value=auto]').attr('checked', 'checked')
      : a.find('.widthType[value=fixed]').attr('checked', 'checked')
    0 == d.width
      ? a.find('.width').parents('.group').hide()
      : a.find('.width').parents('.group').find('.value').html(d.width)
    a.find('.width').slider({
      min: 50,
      max: 900,
      step: 1,
    ...
    d.reflection
      ? a.find('.reflectionOpacity').parents('.group').show()
      : a.find('.reflectionOpacity').parents('.group').hide()
    ...
    d.reflection
      ? a.find('.reflectionBlur').parents('.group').show()
      : a.find('.reflectionBlur').parents('.group').hide()
    ...
    a.find('.reflectionOpacity')
      .parent()
      .find('.value')
      .html(d.reflectionOpacity)
    ...
    a.find('.widthType').click(function () {
      ...
      'auto' == a.find('.widthType[checked]').val()
        ? a.find('.width').parents('.group').hide()
        : a.find('.width').parents('.group').show()
    })
    a.find('.stop').click(function () {
      var c = jQuery(this)
      c.parents('.page').children('.countdown').jCountdown('stop')
      c.hasClass('disable') ||
        (a.find('.slider').slider('disable'),
        a.find('input').attr('disabled', 'disabled'),
        a.find('*').addClass('disable'),
        a.find('.start').parent().removeClass('disable'),
        a.find('.start').removeClass('disable'),
        c.addClass('disable'))
    })
    a.find('.start')
      .click(function () {
        ...
        c.hasClass('disable') ||
          (b.jCountdown('start'),
          a.find('.slider').slider('enable'),
          a.find('input').removeAttr('disabled', 'disabled'),
          a.find('*').removeClass('disable'),
          a.find('.stop').parent().removeClass('disable'),
          a.find('.stop').removeClass('disable'),
          c.addClass('disable'),
          a.find('.create').addClass('disable'))
      })
      .addClass('disable')
    a.find('.destroy').click(function () {
      ...
      a.hasClass('disable') ||
        (c.jCountdown('destroy'),
        b.find('.slider').slider('disable'),
        b.find('input').attr('disabled', 'disabled'),
        b.find('*').addClass('disable'),
        b.find('.create').parent().removeClass('disable'),
        b.find('.create').removeClass('disable'),
        a.addClass('disable'))
    })
    a.find('.create')
      .click(function () {
        ...
        a.hasClass('disable') ||
          (g(c),
          b.find('.slider').slider('enable'),
          b.find('input').removeAttr('disabled', 'disabled'),
          b.find('*').removeClass('disable'),
          b.find('.destroy').parent().removeClass('disable'),
          b.find('.destroy').removeClass('disable'),
          a.addClass('disable'),
          b.find('.start').addClass('disable'))
      })
      .addClass('disable')
  })
  jQuery('.backgroundPicker .button').click(function () {
    for (
      var b = jQuery(this),
        c = b.parents('.page'),
        d = c.children('.countdown'),
        f = c.children('.backgroundPicker'),
        a = [
          'background1',
          'background2',
          'background3',
        ...
        c = c.find('.backgroundPicker .button').index(b),
        e = 0;
      e < a.length;
      e++
    )

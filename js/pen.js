...
const store = {
  ...
  state: {
    config: {
      ...
      shell: 'Crackle',
      size: IS_DESKTOP ? '3' : IS_MOBILE ? '.2' : IS_HEADER ? '1.2' : '2',
      autoLaunch: true,
      finale: false,
      longExposure: false,
      scaleFactor: getDefaultScaleFactor(),
      centerImage: '../images48/svg.png',
    },
  },
  setState(nextState) {
...
function randomColor(options) {
  ...
  if (notSame) {
    while (color === lastColor && attempts < MAX_ATTEMPTS) {
      color = randomColorSimple()
      attempts++
    }
  } else if (notColor) {
    while (color === notColor && attempts < MAX_ATTEMPTS) {
      color = randomColorSimple()
      attempts++
    }
  }
...
function seqPyramid() {
  ...
  function launchShell(x, useSpecial) {
    let shellType = shellTypes[shellNameSelector()]
...
function handleResize() {
  const w = appNodes.stageContainer.offsetWidth
  const h = appNodes.stageContainer.offsetHeight
...
function render(speed) {
  ...
  let bNum = IS_MOBILE ? 0.7 : 1
  // 渲染中心图片效果
  Shell.centerParticles.forEach((particle, index) => {
    const ctx = mainStage.ctx
    particle.life -= speed * 16 // 减少生命周期
    // 检查粒子是否应该移除
    if (particle.life <= 0 || particle.opacity <= 0 || particle.scale > 2) {
      Shell.centerParticles.splice(index, 1) // 移除过期粒子
      return
    }

    particle.scale += 0.01 * speed // 图片逐渐放大
    particle.opacity -= 0.01 * speed // 图片逐渐透明
    ctx.save()
...
class Shell {
  ...
  burst(x, y) {
    ...
    // 添加中心图片粒子
    if (store.state.config.centerImage) {
      ...
      centerParticle.image.src = store.state.config.centerImage

      Shell.centerParticles.push(centerParticle)
    }
  }
}

// 在这里添加静态属性
Shell.centerParticles = []
const BurstFlash = {
  active: [],
  _pool: [],
  _new() {
...
const clickCooldown = 800 // 冷却时间（毫秒）
// 监听点击事件
document
  .querySelector('.new-year-banner')
  .addEventListener('click', (event) => {
    const currentTime = Date.now()
    // 检查是否在冷却时间内
    if (currentTime - lastClickTime < clickCooldown) {
      console.warn('点击太频繁，请稍后再试！')
    ...
    // 将点击位置转换为 Canvas 坐标
    const pos = Stage.windowToCanvas(mainStage.canvas, clientX, clientY)

    // 触发快速烟花效果
    launchQuickFirework(pos.x / mainStage.dpr, pos.y / mainStage.dpr)
  })

// 定义一个在指定位置触发快速烟花的方法
function launchQuickFirework(x, y) {
  const shellType = randomFastShell()
  const shell = new Shell(shellType(shellSizeSelector()))
  shell.launch(x / stageW, y / stageH)
}

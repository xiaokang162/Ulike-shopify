;(function (init) {
  ...
  let yudd = (function () {
    function RainDrop() {
      ...
      $a87C$var$_classCallCheck(this, RainDrop)
      var root = $mrfc$export$default.root,
        controller = $mrfc$export$default.controller
      this.root = root
      this.rate = 1
      ...
      var folder = controller.addFolder('Rain Drop')
      this.datData = controller.addData($a87C$var$data, {
        folder: folder,
      })
    ...
    $a87C$var$_createClass(RainDrop, [
      {
        key: 'setSize',
        value: function setSize() {
          this.rate = Math.min(
      ...
      },
      {
        ...
        value: function update(timestamp) {
          ...
          for (var i = 0; i < positions.length; i += 3) {
            // 控制y坐标从顶部向下变化，模拟雨滴的下落
            positions[i + 1] -= 5 // 每帧让雨滴向下移动
            if (positions[i + 1] < 0) {
              positions[i + 1] = window.innerHeight // 如果雨滴超出屏幕底部，重置位置
              positions[i] = Math.random() * window.innerWidth // 随机水平位置
            }
          }
  ...
  var $m6Ky$export$default = (function () {
    function Text() {
      var _this = this
  ...
  var $Focm$var$WebGL =
    ...
    (function () {
      ...
      $Focm$var$_createClass(WebGL, [
        ...
        {
          key: 'rainShootingStars',
          value: function rainShootingStars() {
            const _this = this

            function createShootingStar() {
              // 从屏幕顶部的任意位置开始
              const startX = Math.random() * $mrfc$export$default.clientWidth // 生成全屏宽度范围的随机 X 坐标
              const startY = -50 // 初始 Y 坐标在屏幕顶部外稍上方

              const driftX = (Math.random() - 0.5) * 200 // 每颗流星的水平漂移，使其有较大范围的横向漂移效果
              const endY = $mrfc$export$default.clientHeight + 50 // 流星的结束位置在屏幕底部稍下方
              const duration = Math.random() * 2000 + 2000 // 每颗流星的下落持续时间（随机值）

              // 使用 animate 方法生成每颗流星的下落动画
              $Zk$export$animate(
                function (progress) {
                  _this.shootingStar.draw({
                    clientX: startX + driftX * (progress / endY), // 随着进度逐渐向下
                    clientY: startY + progress, // 从顶部逐渐向下漂移
                  })
                },
                {
                  begin: 0, // 下落动画的开始位置
                  finish: endY, // 下落动画的结束位置
                  duration: duration, // 设置流星的下落时间
                  easing: 'linear', // 线性下落效果，类似于雨的下落速度
                  onAfter: function () {
                    createShootingStar() // 流星下落结束后，重新创建一个流星
                  },
                }
              )
            }
